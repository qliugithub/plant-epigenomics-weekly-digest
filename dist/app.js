...
function render(){
 ...
 papers.forEach((p,k)=>{const article=el('article','paper');article.append(el('span','paper-number',String(k+1).padStart(2,'0')));const body=el('div');const meta=el('div','meta');meta.append(el('span','badge'+(p.priority==='全文精读'?'':' skim'),p.priority),el('span','',p.journal),el('span','',p.kind),el('span','',p.date));if(p.heading)body.append(el('h3','',p.priority+'｜'+p.heading));body.append(el(p.heading?'h4':'h3','',p.title),meta);const tags=el('div','tags');p.tags.forEach(t=>tags.append(el('span','tag',t)));body.append(tags);const sections=p.sections||[{label:'新在哪里',text:p.novelty},{label:'研究关联',text:p.relevance},{label:'阅读重点',text:p.reading},{label:'研究边界',text:p.limit}];const list=el('ul','analysis-sections');for(const section of sections){const line=el('li','insight');line.append(el('strong','',section.label+'：'),document.createTextNode(section.text));list.append(line)}body.append(list);const a=el('a','','阅读原文 ↗');if(/^https:\/\//.test(p.url)){a.href=p.url;a.target='_blank';a.rel='noopener noreferrer';body.append(a)}article.append(body);$('papers').append(article)});
