...
function render(){
 ...
 papers.forEach((p,k)=>{const article=el('article','paper');article.append(el('span','paper-number',String(k+1).padStart(2,'0')));const body=el('div');const meta=el('div','meta');meta.append(el('span','badge'+(p.priority==='全文精读'?'':' skim'),p.priority),el('span','',p.journal),el('span','',p.kind),el('span','',p.date));body.append(meta,el('h3','',p.title));const tags=el('div','tags');p.tags.forEach(t=>tags.append(el('span','tag',t)));body.append(tags);for(const [label,value] of [['新在哪里',p.novelty],['研究关联',p.relevance]]){const line=el('p','insight');line.append(el('strong','',label+' · '),document.createTextNode(value));body.append(line)}const detail=el('details');detail.append(el('summary','','阅读重点与研究边界'));detail.append(el('p','insight',p.reading),el('p','insight',p.limit));body.append(detail);const a=el('a','','阅读原文 ↗');if(/^https:\/\//.test(p.url)){a.href=p.url;a.target='_blank';a.rel='noopener noreferrer';body.append(a)}article.append(body);$('papers').append(article)});
