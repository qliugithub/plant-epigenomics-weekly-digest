...
assert.equal(run('filteredPapers().length'),13);
assert.equal(run("tags.add('species:拟南芥');tags.add('regulation:H3K27me3');filteredPapers().length"),1);
assert.equal(run("filters.methods='Fiber-seq';filteredPapers().length"),0);
...
for(let i=0;i<13;i++){const paper=run('renderPaper(catalog.papers['+i+'],0,true)');assert.equal(walk(paper).filter(n=>n.tag==='li').length,6);assert(!/[\u4e00-\u9fff]/.test(text(paper)),'Chinese content leaked into English paper '+i);}
...
assert(run('R.ris(catalog.papers.slice(0,1))').includes('TY  - JOUR'));assert(run('R.bibtex([{...catalog.papers[0],title:"A & B_{x}"}])').includes('A \\& B\\_\\{x\\}'));
