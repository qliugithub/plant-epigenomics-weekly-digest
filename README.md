...
## Pepper improvement Journal Club

The **Pepper improvement Journal Club** view presents concise six-field bilingual proposals: trait goal, source strategy/results, pepper applicability and evidence gaps, proposed engineering/outcome, trade-offs/success metrics, and the smallest next action. Traits distinguish disease, abiotic stress, architecture, flowering uniformity/stability/timing, yield, nutrition and fruit traits. Demonstrated interventions in a source species are distinguished from gene-function-inspired hypotheses; neither establishes success in pepper. Conservation remains explicitly unverified pending evidence. Citation count does not determine project priority.

One initial JAG/fruit-shape discussion draft reuses an existing archived paper, explicitly requiring source and orthology checks. It is not a new verified pepper target recommendation. Curated cards live in `data/journal_club.json`; additional cards can be generated with future weekly papers. The new improvement search lane includes genetic engineering/editing and gene-function literature outside epigenomics. Four lanes receive up to nine candidates each (36 maximum). Screening may return null when evidence cannot support an improvement idea. Own multi-omics candidates pursued solely for knockout functional studies are outside this Journal Club.

Add an idea to the **Improvement to-do list**, set discussion/needs-evidence/priority/on-hold, and record owner, review date, rationale and next action. The **Add your own idea** form requires a supporting HTTPS paper link, all six fields, a trait and scope confirmation. These entries and decisions are stored only in the current browser, separate from existing reading notes. They do not synchronize across devices or collaborators. JSON export/import transfers the full local state; import validates the complete backup and keeps newer records without deleting others. CSV exports the currently filtered list. No due-date notifications are created.

Local plans are user-entered in their original language; curated and future generated cards have both languages. The full six-section archived commentary, citation metadata and existing personal reading tools remain available.
